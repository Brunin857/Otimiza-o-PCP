...
try:
    ...
    # Fallback: use UTC offset -3 if tzdata not installed
...
def now_real_h() -> float:
    ...
    t = datetime.now(_BRAZIL_TZ).time()
...
def run_optimizer(cfg: dict, exclude_orders=None) -> bool:
    ...
    NOTE: No st.rerun() needed — with tab_r defined AFTER this function's
    call site in the script, the result is visible immediately in the same pass.
    """
...
def confirm_op(machine_id: str, result, items_res, cfg: dict):
    ...
    now_str   = datetime.now(_BRAZIL_TZ).strftime("%Y-%m-%d %H:%M:%S")
    ...
    with _pt:
        if st.session_state.items_df.empty:
            ...
            st.session_state.items_df = st.data_editor(
                st.session_state.items_df, width="stretch",
    ...
    with _ss2:
        if st.session_state.stock_df.empty:
            ...
            st.session_state.stock_df = st.data_editor(
                st.session_state.stock_df, width="stretch",
...
with tab_c:
    ...
    with _cc1:
        ...
        _trim_mm     = st.number_input("Rebarba por lado (mm)", 0, 50, 10, 1, key="cfg_trim",
                                       help="Perda fixa em cada borda da bobina-mae. Nao conta como desperdicio.")
        st.caption(f"Largura util: Grande = {int(_large_width) - 2*int(_trim_mm)}mm | Pequena = {int(_small_width) - 2*int(_trim_mm)}mm")
    with _cc2:
    ...
    with _cc3:
        ...
        if not st.session_state.maint_df.empty:
            st.session_state.maint_df = st.data_editor(
                st.session_state.maint_df, width="stretch",
...
if st.session_state.get("stock_decision_pending"):
    ...
    with _d1:
        if st.button("🚫 Bloquear pedidos incompletos", use_container_width=True, key="btn_block"):
            run_optimizer(_cfg, exclude_orders=_pedidos_inc)
    with _d2:
...
with tab_r:
    ...
    if result is None:
        ...
        # Metrics
        _n_pulls   = len(result.pulls)
        _trim_mm   = cfg_r.get("trim_mm", 10)
        _trim_total = _n_pulls * 2 * _trim_mm

        _m1, _m2, _m3, _m4, _m5, _m6 = st.columns(6)
        for _col, _lbl, _val, _sub in [
            (_m1, "Puxadas",       f"{_n_pulls}",                          "sequências de corte"),
            (_m2, "Desperdício",   f"{result.total_waste_mm:,.0f}mm",      "sobra de largura útil"),
            (_m3, "Rebarba total", f"{_trim_total:,.0f}mm",                f"{_n_pulls} puxadas × {2*_trim_mm}mm"),
            (_m4, "Setup total",   f"{result.total_setup_min:.0f}min",     "troca de facas"),
            (_m5, "Atraso",        f"{result.total_delay_h:.1f}h",         "⚠️ atrasos" if result.total_delay_h > 0 else "✅ ok"),
            (_m6, "Solver",        result.solver_status,                   "status"),
        ]:
            with _col:
...
with tab_h:
    ...
    if not st.session_state.op_history:
        ...
        st.download_button(
            ...
            file_name=f"flexotimiza_historico_{datetime.now(_BRAZIL_TZ).strftime('%Y%m%d')}.csv",
            mime="text/csv",
        )
