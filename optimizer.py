...
def optimize(
    ...
    # ── Makespan por máquina (substitui total_pulls como proxy de duração) ────
    # Tempo de ciclo por puxada em minutos (igual para todas as puxadas)
    cycle_min = int(params.cycle_time_min * SCALE)
    # Tempo médio de setup por puxada (proxy — sequenciamento exato é feito no greedy)
    avg_setup_min = int(
        (setup.fixed_time_min + setup.time_per_knife_min * (
            sum(len(pat.knife_positions) for pat in flat_patterns) /
            max(len(flat_patterns), 1)
        )) * SCALE
    )
    pull_time_scaled = cycle_min + avg_setup_min  # tempo médio por puxada (escalado)

    # Carga de cada máquina = nº de puxadas × tempo médio por puxada
    machine_load = []
    for m in range(n_m):
        load_m = model.NewIntVar(0, 100_000_000, f"load_{m}")
        model.Add(load_m == sum(use[(p, m)] for p in range(n_p)) * pull_time_scaled)
        machine_load.append(load_m)

    # Makespan = carga máxima entre todas as máquinas
    # Minimizar o makespan força distribuição do trabalho entre as máquinas
    makespan = model.NewIntVar(0, 100_000_000, "makespan")
    model.AddMaxEquality(makespan, machine_load)

    model.Minimize(
        int(alpha) * total_waste +
        int(beta * SCALE) * total_setup +
        int(gamma * 1000) * makespan +   # makespan substitui total_pulls
        int(delta * 100) * total_overprod
    )

    solver.parameters.max_time_in_seconds = time_limit_s
    solver.parameters.log_search_progress = False
    # Hints to find good solution faster
    solver.parameters.num_search_workers = 1   # deterministic on Cloud
