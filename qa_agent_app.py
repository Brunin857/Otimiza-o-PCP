...
# ── Sessão ────────────────────────────────────────────────────────────────────
for k, v in [
    ("phase", "input"),          # input | diagnosing | review | fixing | done
    ("issues", []),              # lista de problemas encontrados
    ("fixes_selected", set()),   # índices dos problemas selecionados para corrigir
    ("fix_results", []),         # resultado de cada correção aplicada
    ("api_key", ""),
]:
...
if st.session_state.phase == "input":
    ...
    with col_k:
        api_key = st.text_input(
            ...
            help="Obtenha em console.anthropic.com. Não é armazenada.",
